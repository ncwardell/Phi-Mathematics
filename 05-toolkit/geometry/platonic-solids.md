...
A regular polygon with n sides has interior angle $(n-2) \times 180°/n$. For at least 3 polygons to meet at a vertex of a Platonic solid, the angle sum must be strictly less than $360°$:

| n-gon | Angle | $3 \times$ Angle | Closes? |
|-------|-------|-----------|---------|
| 3 (triangle) | $60°$ | $180°$ | yes — room to spare |
| 4 (square) | $90°$ | $270°$ | yes — room to spare |
| 5 (pentagon) | $108°$ | $324°$ | yes — barely |
| 6 (hexagon) | $120°$ | $360°$ | no — exactly flat |
| 7+ | $>120°$ | $>360°$ | no — hyperbolic |
...
The non-triangular solids are their duals: Cube $\{4,3\} \leftrightarrow$ Octahedron $\{3,4\}$, Dodecahedron $\{5,3\} \leftrightarrow$ Icosahedron $\{3,5\}$. The tetrahedron $\{3,3\}$ is self-dual.
...
The icosahedral rotation group $A_5$ has a profound algebraic identity:

**$A_5 \cong \text{PSL}(2, 5)$** — the projective special linear group over **$\mathbb{F}_5$**, the field with 5 elements.

This is the bridge between the framework's algebraic completion (F(5) = 5) and classical mathematics:

- $A_5$ is the **smallest non-abelian simple group** (no normal subgroups)
- $A_5$ is the **largest finite subgroup of SO(3)** (the rotation group)
- The simplicity of $A_5$ is **why the quintic equation has no radical solution** (Galois theory)

The algebraic completion at F(5) = 5 is not merely a numerical coincidence. It reflects the deep fact that degree-5 is where polynomial algebra *closes* — no higher-degree equation can be solved by radicals, because the icosahedral group admits no simplification. The field $\mathbb{F}_5$ is the natural home of this closure.
...
|-------------------|-------------|----------------|-----------|
| Tetrahedron | Binary tetrahedral (order 24) | **$E_6$** | $78 = 2 \times 3 \times 13$ |
| Octahedron | Binary octahedral (order 48) | **$E_7$** | $133 = 7 \times 19$ |
| Icosahedron | Binary icosahedral (order 120) | **$E_8$** | $248 = 8 \times 31$ |

The factorizations of these dimensions into framework numbers:

- $\dim(E_6) = 78 = 2 \times 3 \times 13$ = duality $\times$ triangle $\times$ C(Higgs)
- $\dim(E_7) = 133 = 7 \times 19$ = L(4) $\times$ d(bottom) = weak mixing $\times$ anti-Eisenstein norm of (2,3)
- $\dim(E_8) = 248 = 8 \times 31$ = F(6) $\times$ 31

The $E_6$ and $E_7$ factorizations are striking: they use exactly the framework's primitive numbers and derived quantities. This suggests the exceptional Lie algebras may play a direct role in the generation structure.
...
**Gen 2: First dual pair (Cube $\{4,3\} \leftrightarrow$ Octahedron $\{3,4\}$)**
The square face (4-gon) introduces the duality perspective. The cube's 8 vertices = $2^3$ = F(6) = C(W/Z). The octahedron's 12 edges = the witnessing edge number.

**Gen 3: Second dual pair (Dodecahedron $\{5,3\} \leftrightarrow$ Icosahedron $\{3,5\}$)**
The pentagonal face (5-gon) introduces algebraic completion. Both solids are built from $\phi$ — the icosahedron's vertices are $(0, \pm 1, \pm \phi)$ in cyclic permutations. The dodecahedron's diagonal-to-edge ratio is $\phi$. This is the $\phi$-generation: the dual pair where the golden ratio becomes geometrically manifest.
...
Both solids require $\phi$ for their construction. The pentagon's diagonal-to-side ratio IS $\phi$. The triangle-pentagon duality is thus the geometric expression of the framework's fundamental tension between witnessing (3) and self-reference (5), mediated by the golden ratio.

---

## Dependencies

- [Golden Ratio (§1)](/01-foundations/golden-ratio.md) -- $\phi$ appears in icosahedron/dodecahedron vertex coordinates and diagonal ratios
