...
- $\sigma$ = −1 (unfold): $\varphi$-strand expanding, $\psi$-strand contracting
- $\sigma$ = +1 (fold): $\psi$-strand expanding, $\varphi$-strand contracting

The depth circle $S^1$_depth spans from 0 (electron ground state) to F(9) = 34 (last meeting point, spectrum boundary). The breath must complete a full cycle on this circle ($S^1$ topology requires return to starting configuration).
...
## Verification against lepton $\sigma$ values
