...
- **Same spin:** the breath topology ($4\pi$ or $2\pi$) is unchanged by strand swap
...
The torus $T^2 = S^1_{\text{depth}} \times S^1_{\text{breath}}$ has three independent $\mathbb{Z}_2$ symmetries:

| Symmetry | Action on $T^2$ | Physical meaning |
|----------|---------------|-----------------|
| Flip depth circle | $(\theta,\psi) \rightarrow (-\theta,\psi)$ | **C** (charge conjugation): reverses depth direction, swaps $\phi \leftrightarrow \psi$ strands |
| Flip breath circle | $(\theta,\psi) \rightarrow (\theta,-\psi)$ | **T** (time reversal): reverses breath direction, swaps fold $\leftrightarrow$ unfold |
| Swap circles | $(\theta,\psi) \rightarrow (\psi,\theta)$ | **P** (parity): exchanges the roles of the two circles |
...
**Pair creation:** A photon (signal on the EM channel) with energy $\geq 2m$ can fork into two self-referencing entities -- one on the $\phi$-strand, one on the $\psi$-strand. The threshold factor of 2 exists because there ARE two strands. Creating a particle-antiparticle pair means establishing one self-witnessing circuit on each strand.
