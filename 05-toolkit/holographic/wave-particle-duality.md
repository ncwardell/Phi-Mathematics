...
*(ii) The transverse projection of gamma (looking from the side) is a sinusoid of amplitude R_2 and wavelength 2piR_2 -- the WAVE aspect (extended, oscillating).*

*(iii) The two projections are Fourier conjugates: Delta(circle position) x Delta(wave frequency) >= 1/2, which IS the Heisenberg uncertainty relation Delta_x . Delta_p >= hbar/2.*

*(iv) The ratio of particle-scale to wave-scale = phi^d at depth d, so heavier particles appear more classical.*

*(v) These projections correspond to the phi-strand (depth circle) and psi-strand (breath circle), which are conjugate by phi psi = -1.* QED

**Proof sketch.** The torus path parameterized in R^3:

x(t) = (R_1 + R_2 cos omega_b t) cos omega_d t,   y(t) = (R_1 + R_2 cos omega_b t) sin omega_d t,   z(t) = R_2 sin omega_b t

Project onto (x,y) plane (suppress z): you see a particle orbiting in a thick circle (annulus), radius R_1 +/- R_2. This is the **particle** -- localized, with definite angular position.
...
The two projections are related by Fourier transform (angular position <-> angular frequency), so they obey the Fourier uncertainty theorem: Delta_theta . Delta_omega >= 1/2. With hbar restoring units, this is Heisenberg. QED
...
E/p = (omega_d/R_1) / (omega_b/R_2) = (omega_d/omega_b) x (R_2/R_1) = (omega_d/omega_b) / phi
...
**The quantum-classical boundary.** At depth d, the circle circumference exceeds the wave amplitude by phi^d. At d = 0 (electron), the ratio is 1 -- equally wave and particle. At d = 11 (muon), the ratio is phi^11 ~ 199 -- mostly particle. At d = 26 (top quark), the ratio is phi^26 ~ 271,443 -- overwhelmingly particle. Heavier particles appear more classical because the circle projection dominates at higher depths. Quantum effects are most visible for light particles (electrons, photons) because their wave and particle scales are comparable.
