...
E_rest = m_e x phi^d x (1 + C alpha(1+4alpha)) x c^2

Mass and rest energy are the same thing: the energy required to maintain a self-referencing circuit of d witnessing steps.

**Velocity on the torus.** A particle "at rest" has its helix wrapping both torus circles at equal rates (omega_depth = omega_breath). A particle "moving" has unequal rates. From the flow equations (Theorem 41):

**v/c = tanh(eta)**

where eta is the rapidity -- the tilt angle of the helix on the torus. The fundamental velocity v_1 = c/sqrt(5) corresponds to eta = ln(phi), one golden-ratio step of tilt. A particle cannot reach c because tanh(eta) < 1 for all finite eta -- the helix can tilt but never flatten completely.
...
**F = ma = (m_e x phi^d x correction) x a**
...
T = E - m = m(cosh(eta) - 1) ~ (1/2)mv^2 for v << c

**This resolves Part IV Open Question 1 completely.** The cosh(eta) "temporal correction" in the mass formula IS kinetic energy. When a particle is moving (eta != 0), its effective energy increases by the factor cosh(eta). The correction applies when:
- Leptons at rest: eta = 0, cosh(eta) = 1, no correction needed -> clean mass formula
- Quarks inside hadrons: eta > 0 (they carry momentum), cosh(eta) > 1 -> correction needed
...
G ~ phi^(-214) ~ 1.89 x 10^(-45) (natural units)
...
| v = c tanh(eta) | Theorem 41 flow equations | Velocity = helix tilt |
...
| E^2 = p^2 + m^2 | Theorem 44 Casimir | Mass-shell from sl(2,R) |
