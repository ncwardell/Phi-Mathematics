...
The Kepler triangle provides the geometric constraint. A right triangle with sides $(1, \sqrt{x}, x)$ satisfying the Pythagorean theorem:

$1^2 + (\sqrt{x})^2 = (x)^2$ — but this requires $x = x^2$, which gives only $x = 1$ (trivial).
