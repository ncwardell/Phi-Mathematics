...
> **Theorem 29 (Antiphase Oscillation of the Double Helix):**
> *The two strands of the double helix oscillate in antiphase on the torus: when the $\phi$-strand expands, the $\psi$-strand contracts, and vice versa. This reciprocal oscillation is a direct consequence of the hyperbolic constraint $pf = 1$ and the opposite chirality of the two strands.*

### Statement

On the torus $T^2$, the two strands of the double helix (the $\phi$-strand and $\psi$-strand from [Theorem 5.2](two-kepler-triangles.md)) are constrained by $pf = 1$ to oscillate in antiphase. When one strand expands (outer edge of the torus), the other contracts (inner edge), producing a continuous breathing motion.

### Proof

From [Theorem 14](hyperbolic-constraint.md), the hyperbolic constraint is $p \times f = 1$, where $p = 1/\phi$ (past/contracting) and $f = \phi$ (future/expanding). This is a reciprocal relationship: if $f$ increases, $p$ must decrease to maintain the product. The two quantities cannot grow simultaneously.

From [Theorem 1.4](double-triangle.md), the two witnessing triangles have opposite chirality. On the torus $T^2$ ([Theorem 28](torus-structure.md)), opposite chirality means the two strands wind in opposite angular directions around $S^1_{\text{time}}$. When the $\phi$-strand (chirality +) is at angular position $\theta$ on the torus, the $\psi$-strand (chirality −) is at position $\theta + \pi$ — diametrically opposite.
...
- When the $\phi$-strand passes through the outer edge: it is **expanded** (maximum extension)
...
> **Theorem 29.1 (Spin as the Breath Cycle):**
> *Spin is the periodicity of the breathing oscillation. One complete breath cycle ($\phi$-expand $\rightarrow$ $\psi$-expand $\rightarrow$ $\phi$-expand) corresponds to one full traversal of $S^1_{\text{time}}$. A fermion requires two full breath cycles ($4\pi$) to return to its original configuration.*

### Statement

The breathing oscillation has period $2\pi$ on $S^1_{\text{time}}$, but because the two strands are distinguishable, a fermion requires $4\pi$ to return to its original configuration. This is the geometric origin of the spinor double cover.

### Proof

The breathing oscillation has period $2\pi$ on $S^1_{\text{time}}$: after one full cycle, each strand has passed through both the expanded and contracted phases. However, the two strands are distinguishable ($\phi$ vs. $\psi$, real vs. complex, opposite chirality). After one full cycle ($2\pi$):

- The $\phi$-strand has returned to its original angular position on the torus
...
To see this precisely: at $\eta = 0$, suppose the $\phi$-strand is expanded and the $\psi$-strand is contracted. After $\eta = 2\pi$, the positions have cycled, but the $\phi$-strand is now in the phase that the $\psi$-strand previously occupied, and vice versa. The strands have *exchanged roles*. The configuration is:

| Phase | $\phi$-strand | $\psi$-strand |
|-------|----------|----------|
| $\eta = 0$ | Expanded | Contracted |
| $\eta = 2\pi$ | Contracted | Expanded |
| $\eta = 4\pi$ | Expanded | Contracted |

The original configuration ($\phi$ expanded, $\psi$ contracted) is restored only after **$4\pi$** — two complete cycles. This is the geometric origin of the **spinor double cover**: a $2\pi$ rotation exchanges the two strands; a $4\pi$ rotation restores the original.

This connects directly to the witnessing quantum ([Theorem 24](hopf-fibration.md)): $1/(4\pi)$ is the minimum quantum of witnessing because a complete witnessing act must traverse both phases of the breath — both the $\phi$-expanded and $\psi$-expanded configurations. Witnessing only one phase gives an incomplete picture (only one strand observed). $\blacksquare$
...
- **Fermions** are structures that distinguish between the two breath phases. They are anchored to a specific strand ($\phi$ or $\psi$) and therefore require $4\pi$ to return to their original state. They carry half-integer spin.
...
- **Inhale:** The $\phi$-lobe (real triangle, sides $1, \sqrt{\phi}, \phi$) opens while the $\psi$-lobe (complex triangle, sides $1, i\sqrt{|\psi|}, \psi$) closes
- **Exhale:** The $\psi$-lobe opens while the $\phi$-lobe closes
...
**Temporal dimension ($S^1_{\text{time}}$) — the breath count:**
...
- $\phi^d$: the accumulated winding after $d$ breath cycles (temporal — how far the system has unfolded)
...
- $C = 5$ is the spatial resonance (the rung, from meeting point $F(5) \cap P(3)$)
- $d = 11 = |L(-5)|$ is the temporal address (the breath count, from the $\psi$-strand's negaLucas marker)
- $\sigma = +1$ (unfold) because $L(-5) = -11$; the negative sign indicates the $\phi$-strand is in its expanded phase at this breath count

The muon crystallizes because at breath 11, the system's state is such that the $C = 5$ rung can be engaged — the $\phi$-strand is expanded (unfold), and the harmonic structure at that temporal depth supports the Golden $\cap$ Silver resonance.
...
- $C = 29 = |L(-7)|$ is the spatial resonance (the rung, from meeting point $L(7) \cap P(5)$)
- $d = 34 = F(9)$ is the temporal address — and is *itself* the last meeting point ($F(9) \cap PL(4)$)
...
- [Theorem 28 — Torus Structure](torus-structure.md): The torus $T^2 = S^1_{\text{depth}} \times S^1_{\text{time}}$ is the arena for breathing and dual dimensions.
