...
The Fibonacci recurrence F(n) = F(n−1) + F(n−2) defines an ordered sequence indexed by n ∈ ℤ. The index n provides all the structural properties of time — ordering, asymmetry, locality, and progression — without any external assumption.

## Proof

The Fibonacci recurrence F(n) = F(n−1) + F(n−2) defines an ordered sequence indexed by n ∈ ℤ.

1. **Ordering:** n provides a total order. For any two states F(a) and F(b), either a < b, a = b, or a > b.

2. **Asymmetry (arrow of time):** F(n+1)/F(n) → φ > 1 for positive n. The sequence grows in the forward direction and decays in the backward direction. The forward direction is distinguished — it is the direction in which self-reference *unfolds* rather than *collapses*.
...
Time is not a background parameter. It is the structure of the dynamic self-reference itself. "Now" is the current depth of the unfolding. ∎
...
- [Theorem 28 — Torus Structure](torus-structure.md): Time defines one of the two circles S¹_time of the torus.
