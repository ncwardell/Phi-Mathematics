...
The minimal stable witnessing structure is not a single triangle but a pair of triangles of opposite chirality: the $\varphi$-triangle {A, B, C} and the $\psi$-triangle {−A, −B, −C}, connected by a shared unit of distinction.
...
If the $\varphi$-triangle has the directed cycle A → B → C → A, the $\psi$-triangle has the reversed cycle (−A) → (−C) → (−B) → (−A). The negation map on an oriented 3-cycle reverses the cyclic order because (−1) $\times$ permutation sign flips orientation. Applying negation to each element preserves the edges but inverts the handedness of the cycle as a whole. The two triangles are mirror images — enantiomers of the same witnessing structure.
...
|-----------|----------|-----------|------|
| $\varphi$-triangle | {A, B, C} | + (e.g., clockwise) | Real/visible strand |
