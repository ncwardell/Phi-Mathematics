...
The minimal stable witnessing structure is not a single triangle but a pair of triangles of opposite chirality: the φ-triangle {A, B, C} and the ψ-triangle {−A, −B, −C}, connected by a shared unit of distinction.

## Proof

By Theorem 1 (Polarity), every element that exists must have a polar complement. If {A, B, C} exist to form the witnessing triangle, then {−A, −B, −C} must also exist. This is not optional — it is forced by Conservation (Σ = 0).

**The anti-triangle has reversed chirality:**
If the φ-triangle has the directed cycle A → B → C → A, the ψ-triangle has the reversed cycle (−A) → (−C) → (−B) → (−A). The negation map on an oriented 3-cycle reverses the cyclic order because (−1) × permutation sign flips orientation. Applying negation to each element preserves the edges but inverts the handedness of the cycle as a whole. The two triangles are mirror images — enantiomers of the same witnessing structure.
...
|-----------|----------|-----------|------|
| φ-triangle | {A, B, C} | + (e.g., clockwise) | Real/visible strand |
| ψ-triangle | {−A, −B, −C} | − (counterclockwise) | Complex/hidden strand |

**Conservation is satisfied:**
A + (−A) + B + (−B) + C + (−C) = 0. The six elements sum to zero, satisfying Σ = 0.
...
The *relationship* between the two triangles — the fact that they are polar complements of each other — constitutes the unit of distinction "1". This is the bridge: not a seventh element, but the structural relation connecting the two triads. Each triangle defines itself partly in contrast to the other. ∎
...
**Corollary 1.4.2 (Six-fold Structure):** The number 6 = 2 × 3 is the minimal element count for a complete, conserved, stable witnessing structure: 3 for stability (Theorem 1.3), doubled by polarity (Theorem 1).
...
- [Axiom (Conservation)](axioms-and-postulates.md) — Σ = 0 is satisfied by the six-element structure
