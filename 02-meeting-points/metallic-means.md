...
$x = k + 1/x \rightarrow x^2 - kx - 1 = 0 \rightarrow x = (k + \sqrt{k^2 + 4})/2$
...
The dominant eigenvalue (growth rate) of each recurrence is the metallic mean itself. Higher $k$ $\rightarrow$ faster growth $\rightarrow$ higher frequency winding around the torus.

Each metallic mean also has a Lucas-type companion:

$ML_k(n) = k \cdot ML_k(n-1) + ML_k(n-2)$
...
The system's complete integer output is the union of all these sequences, generated progressively as new metallic families are born. $\blacksquare$
