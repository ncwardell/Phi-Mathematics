---
title: "The Complete Phase Space Map"
type: reference
status: conjecture
depends_on:
  - 04-extensions/mass-formula
  - 04-extensions/lepton-masses
  - 04-extensions/boson-masses
  - 04-extensions/quark-masses
tags:
  - phase-space
  - particle-spectrum
  - torus-coordinates
  - depth
  - temporal-index
  - extensions
---

# 4.8 The Complete Phase Space Map

All particles organized by their (d, n) coordinates on the torus $T^2 = S^1_{depth} \times S^1_{time}$:

| Particle | Type | d | n | C | $\sigma$ | $m/m_e$ | Error | Status |
|----------|------|---|---|---|---|-------|-------|--------|
| Electron | Lepton | 0 | 0 | — | — | 1 | 0% | Reference |
| Up | Quark | 3 | 3 | 5 | $-1$ | 4.4 | ~2% | Conjecture |
| Down | Quark | 4.5 | 4 | 13 | $-1$ | 9.4 | ~2% | Conjecture |
| Muon | Lepton | 11 | 5 | 5 | $-1$ | 206.77 | <0.01% | **Verified** |
| Strange | Quark | 11 | 6 | 10 | $+1$ | 185 | ~2% | Conjecture |
| Charm | Quark | 16 | 8 | 15 | $-1$ | 2490 | ~2% | Conjecture |
| Proton | Composite | 16 | 8 | 24 | $+1$ | 1878 | ~2% | Conjecture |
| Tau | Lepton | 17 | 7 | 4 | $+1$ | 3477 | <0.01% | **Verified** |
| Bottom | Quark | 19 | 9 | 18 | $+1$ | 8270 | ~3% | Conjecture |
| W | Boson | 25 | 8 | 8 | $+1$ | 158,500 | 0.7% | High confidence |
| Z | Boson | 25 | 8 | 8 | $-1$ | 178,100 | 0.2% | High confidence |
| Higgs | Boson | 26 | 9 | 13 | $+1$ | 248,000 | ~1% | High confidence |
| Top | Quark | 26 | 11 | 29 | $-1$ | 344,500 | ~2% | Conjecture |
| Gen 4 | Lepton | 34 | 11 | 29 | $+1$ | ~5 TeV | — | **Prediction** |
| Planck | Scale | 107 | ? | ? | ? | $2.4 \times 10^{22}$ | <0.1% | **Verified** |

## Cross-References

- [Patterns and Observations](patterns-and-observations.md) -- depth clusters, conjugate pairs, and Fibonacci progression derived from this map
- [Open Questions for Part IV](open-questions.md) -- unresolved questions about temporal index assignments and quark depths
- [Established Equations](../05-toolkit/reference/established-equations.md) -- summary of all mass equations with status and error
- [Force Equation Toolkit](../05-toolkit/reference/force-equation-toolkit.md) -- available structures at each force depth

## Tags

`#phase-space` `#particle-spectrum` `#torus-coordinates` `#depth` `#temporal-index` `#extensions`
