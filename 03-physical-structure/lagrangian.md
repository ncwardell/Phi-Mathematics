...
> *The Lagrangian that reproduces the flow equations (Theorem 41) through the Euler-Lagrange equations is L = pf_dot - H = pf_dot - (pf - m^2). In spacetime coordinates, this yields the standard relativistic action for a massive particle.*
...
We seek a Lagrangian L(p, f, f_dot) such that the Euler-Lagrange equations

d/deta (dL/df_dot) - dL/df = 0

reproduce f_dot = f (and correspondingly for p).

**Construction:** Define:

L = pf_dot - H = pf_dot - pf + m^2

This is a Legendre transform from the Hamiltonian, treating p as the "momentum" conjugate to f. Computing:

dL/df_dot = p --> d/deta(dL/df_dot) = p_dot = -p
dL/df = -p (from the -pf term, treating p as independent)
...
f = e^eta = cosh(eta) + sinh(eta) --> relates to t + x
p = e^(-eta) = cosh(eta) - sinh(eta) --> relates to t - x

Under this transformation, the action becomes:

S = integral L deta = -m integral dtau sqrt(t_dot^2 - x_dot^2)

where tau is the proper time parameter and t_dot = dt/dtau, x_dot = dx/dtau.
...
- [Lorentz Algebra (Theorem 44)](lorentz-algebra.md) -- the Lagrangian possesses sl(2,R) symmetry
