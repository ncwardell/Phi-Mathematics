...
**Unfold ($\sigma$ = +1, $\varphi$-strand expanded at crystallization):**
m/$m_e$ = $\varphi$ᵈ / (1 − C·$\alpha$·(1 + 4$\alpha$))

**Fold ($\sigma$ = −1, $\psi$-strand expanded at crystallization):**
m/$m_e$ = $\varphi$ᵈ / (1 + C·$\alpha$·(1 + 4$\alpha$)/(1 + 5π$\alpha$))

| Component | Origin | Theorem |
|-----------|--------|---------|
| $\varphi$ᵈ | Accumulated winding after d breath cycles | Theorem 12, 29.3 (temporal) |
| C | Coupling coefficient from meeting point | Theorem 34, 29.3 (spatial) |
| $\alpha$ | EM bridge coupling | Theorem 47 (§3.9.1) |
| 4$\alpha$ = L(3)·$\alpha$ | Witnessing correction from hourglass geometry | Theorem 24 (L(3) = 4) |
| $\sigma$ = ±1 | Breath phase from negasequence sign | Theorem 13, 29 |
| 5π$\alpha$ (fold denominator) | Hopf correction for $\psi$-strand engagement | Theorem 24 (5 = F(5), $\pi$ from fiber) |
...
- C = 5 = F(5): muon, from meeting point (Golden ∩ Silver); algebraic completion ($\sqrt{5}$)
...
- Muon: $\sigma$ = +1 (unfold). L(−5) = −11; $\varphi$-strand expanded at breath 11.
...
- [Hopf Fiber (Theorem 24)](../01-foundations/hopf-fiber.md) -- witnessing correction L(3)=4 and Hopf factor 5π$\alpha$
- [Negasequences (Theorem 13)](../01-foundations/negasequences.md) -- sign structure $\sigma$ = ±1
