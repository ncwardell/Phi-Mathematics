...
From Theorem 14, the hyperbolic constraint is p x f = 1 for the ground state (electron, m = 1 in natural units). For a general self-referential structure with mass m, the constraint generalizes to:

p x f = m^2

This follows because mass scales the depth on the torus (Theorem 29.3). A structure with mass m has accumulated m^2 units of winding (the square arising from the product of the two strand contributions -- phi-strand x psi-strand, consistent with the double helix).
...
- Theorem 14 (Hyperbolic Structure) -- provides the ground-state constraint p x f = 1
- Theorem 29.3 (Breathing Torus / Depth) -- mass scales the depth, giving p x f = m^2
...
- [Lorentz Algebra (Theorem 44)](lorentz-algebra.md) -- sl(2,R) is the symmetry group of the hyperbolic constraint
